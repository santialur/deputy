...
directiveModule.directive('ngDropdownMultiselect', ['$filter', '$document', '$compile', '$parse', function($filter, $document, $compile, $parse) {
	return {
		...
		template: function(element, attrs) {
			...
			template += '<li class="multiselect-top" ng-if="settings.showUncheckAll"><span>{{getSelected()}}</span> <a ng-keydown="keyDownLink($event)" data-ng-click="deselectAll();" tabindex="-1" id="deselectAll"></span>{{texts.uncheckAll}}</a></li>';
			...
			template += '<i class="fa fa-check" data-ng-if="isChecked(getPropertyForObject(option,settings.idProp))"> </i> <span ng-class="{\'padded\': !isChecked(getPropertyForObject(option,settings.idProp))}" dm-dropdown-static-include="{{settings.template}}"></span></a>';
